...
if ( ! class_exists( 'CTC_Shortcodes' ) ) {
	class CTC_Shortcodes {
		...
		function group_shortcode( $type, $attr) {
			// $type hierarchy
...
<?php				
		...
		function people_shortcode( $attr ) {
			return group_shortcode( 'people', $attr ); 
...
<?php
		...
		public function events_shortcode( $attr ) {
			$output = apply_filters( array( &$this, 'events_shortcode' ), '', $attr );
			if ( $output != '' ) return $output;
			extract( shortcode_atts( array(
				'before' 	=>	'',
				'after' 	=>	'',
		...
		// Sermon
		public function sermons_shortcode( $attr ) {
			$output = apply_filters( array( &$this, 'sermons_shortcode' ), '', $attr );
			if ( $output != '' ) return $output;
			extract( shortcode_atts( array(
				'before' 	=>	'',
				'after' 	=>	'',
